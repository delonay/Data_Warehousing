...
SELECT NON EMPTY {[Measures].[Quantity - Tb Transactions],
       ...
       NON EMPTY {[Tb Supplier].[State].[State].ALLMEMBERS
				 } ON ROWS 
  FROM DS715


/*3)Number of transactions between supplier-city-consumer-city pairs?*/
SELECT NON EMPTY {[Measures].[Tb Transactions Count]
				 } ON COLUMNS,
       NON EMPTY {([Tb Supplier].[City].[City].ALLMEMBERS*
...
SELECT NON EMPTY {[Measures].[Quantity - Tb Transactions]
       ...
       NON EMPTY {([Tb Product].[Name].[Name].ALLMEMBERS*
	               [Tb Supplier].[State].[State].ALLMEMBERS
				  )
				 } ON ROWS 
  FROM DS715
...
SELECT NON EMPTY {[Measures].[Quantity - Tb Transactions]
       ...
       NON EMPTY {[Tb Consumer].[Name].[Name].ALLMEMBERS			  
				 } ON ROWS 
  FROM DS715
  WHERE ([Tb Supplier].[Name].&[Bernstein],
...
/*11)(Extra Credit) For each product list quantity sold by suppliers in Madison to */
/*consumers inChicago versus quantity sold by suppliers in Chicago to consumers in*/
/* Madison (result columns will be: product name, quantity Madison_Chicago, quantity Chicago_Madison?*/
